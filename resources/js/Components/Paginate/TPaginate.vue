<template>
  ...
  <path fill-rule="evenodd"
    ...
    <div :class="countersStyle">
      <div
          :disabled="range>modelValue"
          :class="previousArrowStyle"
          @click="previousPage"
      >
      ...
      <template v-for="item in dynamicRange">
        <div
      ...
      <div
          :disabled="Math.ceil(modelValue.value/range.value) === Math.ceil(total.value/range.value)"
          :class="nextArrowStyle"
          @click="nextPage"
      >
...
export default defineComponent({
  ...
  setup(props, {emit, expose}) {
    /*Definitions*/
    const {total, radius, design, color, modelValue, range, detailText} = toRefs(props)
  ...
  const detailGeneratedText = () => {
    let finalText
    finalText = detailText.value.replace('$a', modelValue.value)
    finalText = finalText.replace('$b', total.value)
  ...
  const previousArrowStyle = computed(() => {
