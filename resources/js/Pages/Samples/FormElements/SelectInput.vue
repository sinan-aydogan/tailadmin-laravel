<template>
  <app-layout :actionButtons="true">
    ...
    <template #action-buttons>
      <TButton color="yellow" :radius="3">
        <font-awesome-icon icon="angle-left"/>
        Back to Home
      </TButton>
      <TButton color="green" :radius="3">
      ...
      <t-form-content @submitted="save" @reset="reset">
        <t-form-section
          ...
          <t-input-group label="Manager - Searchable" labelFor="user_id" class="col-span-12 md:col-span-6">
            <t-input-dropdown v-model="form.user_id" :clear-button="true" place-holder="Select"
                              search-place-holder="Search...">
              <t-input-dropdown-item v-for="(item,index) in users" :value="item.id" :key="index"><span
                  :class="item.class">{{ item.name }}</span></t-input-dropdown-item>
            </t-input-dropdown>
          </t-input-group>
          <!-- Tax ID -->
